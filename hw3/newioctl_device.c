...
void get_ioctl_helper(struct ioctl_arg *msg){
	...
	if (msg->vector == 0) {
		if (task->sys_vector == NULL) {
			printk
				("Process does not have any module linked RETURNING FROM HERE");
			
			return;
		}
		decrement_module_rc(((struct module_reg *)task->
					sys_vector->module_addr)->type);
		...
		if (task->sys_vector == NULL) {
			task->sys_vector =
				...
				kmalloc(sizeof(struct new_sys_vector),
						GFP_KERNEL);
			increment_module_rc(msg->module_type);
		} else if (task->sys_vector->module_addr != addr) {
			decrement_module_rc(((struct module_reg *)task->
						sys_vector->module_addr)->
					type);
			increment_module_rc(msg->module_type);
		}
...
/*			 pid_struct = find_get_pid(msg->pid);
			...
			if (msg->vector == 0) {
				if (task->sys_vector == NULL) {
					printk
						("Process does not have any module linked");
					goto out;
				}
				decrement_module_rc(((struct module_reg *)task->
							sys_vector->module_addr)->type);
...
int setup_ioctl_dev()
{
	...
	/*if (ndevices <= 0)
	  {
	  printk(KERN_WARNING "[target] Invalid value of ndevices: %d\n",
	  ndevices);
	  err = -EINVAL;
	  return err;
	  } */
